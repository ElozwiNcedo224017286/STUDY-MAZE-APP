...
// Study Maze - QuizAPI Question Service
// ============================================================
//
// QuizAPI response is converted into the format used by
// the Study Maze game:
//
// {
//   id: '...',
...
//   difficulty: 'easy',
//   q: 'Question text',
//   opts: ['Answer 1', 'Answer 2', ...],
//   correct: 0
// }
//
// QuizAPI documentation:
...
// ============================================================
//
// Your Maze Game subjects
//        ↓
// QuizAPI categories
//
// QuizAPI does not necessarily have an exact category for every
// subject in your game, so we map the closest available category.
//
// ============================================================
...
function transformQuizApiQuestion(question) {
  if (!question) {
    return null;
  }
  ...
  if (!question.text || answers.length === 0) {
    return null;
  }
  ...
  // We cannot use the question if QuizAPI did not provide
  // a valid correct answer.
  if (correctIndex === -1) {
    return null;
  }

  const difficulty = normalize(
    question.difficulty
  ).toLowerCase();
  ...
  return {
    ...
    // Keep the original QuizAPI information available.
    // This does not interfere with the Maze Game.
...
export async function fetchQuizApiQuestions({
  subject = null,
  difficulty = null,
  limit = 10,
  offset = 0,
  ...
  const categories =
    SUBJECT_CATEGORY_MAP[normalizedSubject];
  ...
  // ----------------------------------------------------------
  //
  // Your Maze Game expects answer options.
  //
  // ----------------------------------------------------------
  ...
  const url =
    `${QUIZAPI_BASE_URL}/questions?${params.toString()}`;
  ...
  try {
    ...
    throw new Error(
      `QuizAPI returned an invalid response. HTTP ${response.status}`
    );
  }
  ...
  if (!response.ok) {
    throw new Error(
      result?.error ||
      `QuizAPI request failed with HTTP ${response.status}`
    );
  }
  ...
  if (!result?.success) {
    throw new Error(
      result?.error ||
      'QuizAPI request was not successful.'
    );
  }
...
// ============================================================
//
// This is the function your existing MazeGameScreen can use.
//
// Example:
//
// const questions = await getQuizBank(
//   'MATHEMATICS',
//   'easy'
// );
//
...
export async function getQuizBank(
  ...
  try {
    ...
    console.log(
      `[QuizAPI] Received ${questions.length} questions`
    );

    return questions;
  } catch (error) {
    console.error(
      '[QuizAPI] getQuizBank failed:',
      error
    );
...
// ============================================================
//
// This keeps the same interface as your original code.
//
// customQuestions can be questions returned by QuizAPI,
// a teacher API, Supabase, etc.
//
// ============================================================
...
// ============================================================
// FETCH + BUILD ACTIVE POOL
// ============================================================
//
// This is the easiest function to call from your Maze Game.
//
// It:
//
// 1. Calls QuizAPI
// 2. Gets questions
// 3. Converts them to Maze format
// 4. Filters subject
// 5. Filters difficulty
// 6. Shuffles the questions
//
// ============================================================

export async function getActiveQuestionPool(
  subject = null,
  difficulty = null,
  limit = 20
) {
  ...
  return buildActiveQuestionPool(
    questions,
    subject,
    difficulty
  );
